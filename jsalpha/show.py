...
def show_result(daily_return, period, benchmark_daily_return, benchmark_title):
    ...
    sharpe_ratio, max_drawdown, annual_return, _, total_return = utils.metrics(daily_return, period)
    bench_sharpe, bench_max_drawdown, bench_annual_return, _, bench_total_return = utils.metrics(benchmark_daily_return, period)
    ...
    fig_data = [
        ...
        (0.75, label_height, value_height, "MaxDrawdown", "{0:.3%}".format(metrics_dict["max_drawdown"]), red, black),
        (0.75, label_height2, value_height2, "Benchmark MaxDrawdown", "{0:.3%}".format(metrics_dict["benchmark_max_drawdown"]), blue, black),
    ]
